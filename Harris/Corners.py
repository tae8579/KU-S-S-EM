...
def findCorners(img, window_size, k, thresh):
    ...
    for y in range(int(offset), int(height-offset)):
        for x in range(int(offset), int(width-offset)):
            ...
            windowIxx = Ixx[y-int(offset):y+int(offset)+1, x-int(offset):x+int(offset)+1]
            windowIxy = Ixy[y-int(offset):y+int(offset)+1, x-int(offset):x+int(offset)+1]
            windowIyy = Iyy[y-int(offset):y+int(offset)+1, x-int(offset):x+int(offset)+1]
            Sxx = windowIxx.sum()
            Sxy = windowIxy.sum()
            Syy = windowIyy.sum()
...
def main():
    ...
    area = (60, 60, 760, 520)
    cropped_img = img.crop(area).save('temp.png')
    ...
    if img is not None:
        ...
        if finalImg is not None:
            cv2.imwrite("Images/result.png", finalImg)
        ...
        result_img = Image.open('Images/result.png')
